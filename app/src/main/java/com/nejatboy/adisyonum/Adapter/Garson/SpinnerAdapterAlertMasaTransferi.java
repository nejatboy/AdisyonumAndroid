...
public class SpinnerAdapterAlertMasaTransferi extends ArrayAdapter<Garson> {

    public SpinnerAdapterAlertMasaTransferi(Context context, List<Garson> garsonlar) {
        super(context, 0, garsonlar);
    }
    ...
    private View initView (int position, View convertView, ViewGroup parent) {      //Hazır metod değil ben yazdım (override metodlara return edecek)
        Garson garson = getItem(position);
        ...
        if (garson != null) {
            ...
            textView.setText(garson.getGarsonAd());
        }
